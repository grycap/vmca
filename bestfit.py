...
class Defragger_Refill_Relative(defragger.Defragger_Base):
    def _migration_enhancement(self, r_mean, hosts_info, h_id, res_amount):
        free_before = hosts_info.euclid_normalized_resources_free(h_id)
        total = hosts_info.euclid_normalized_resources_total(h_id)
        free_before_r = free_before / total
        res_amount_r = res_amount / total
        distance_to_mean_before = math.fabs(r_mean - free_before_r)
        distance_to_mean_after = math.fabs(r_mean - (free_before_r + res_amount_r))
        return distance_to_mean_before - distance_to_mean_after
    ...
    def defrag(self, _hosts_info, hosts_fixed = [], fixed_vms = []):
        ...
        for h_id in hosts_info.keys():
            # r_total += float(hosts_info.euclid_normalized_resources_free(h_id))
            r_total += float(hosts_info.euclid_normalized_resources_free_relative(h_id))
        ...
        hosts_resource_info = []
        for h_id, host in hosts_info.items():
            hosts_resource_info.append((h_id, hosts_info.euclid_normalized_resources_free_relative(h_id)))
            for vm in host.vm_list[:]:
                if vm.id not in fixed_vms:
                    vm_norm_res = hosts_info.calculate_euclid_normalized_resources(vm.memory, vm.cpu)
                    possible_migrable_vm.append((vm, vm_norm_res))
        
        # We'll sort the resources so as the one that has less free resources is tried to be filled first
        # - that tries to make that if the platform is very empty, some hosts with a lot of free resources will be tried to kept free
        #   to be able to power them off
        hosts_resource_info = sorted(hosts_resource_info, key = lambda x: x[1], reverse=False)
            ...
            while continue_moving:
                ...
                for (vm, vm_norm_res) in possible_migrable_vm:
                    ...
                    if (not host.has_vm(vm)) and host.vm_can_fit(vm):
                        ...
                        enhancement = self._migration_enhancement(r_mean, hosts_info, h_id, -vm_norm_res)
                        if enhancement > 0:
                            possible_vms.append((vm, float(hosts_info.euclid_normalized_resources_free_relative(h_id)) - enhancement))
